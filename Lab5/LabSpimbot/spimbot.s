# syscall constants
#read writeup on how to avoid
#function: avoid_banana
...
jetstream_x: .space 4
jetstream_x2: .space 4
jetstream_y: .space 4
jetstream_y2: .space 4
jetstream_x2y2: .space 4
SCAN_FLAG: .space 4
sav_s1: .space 4
sav_s2: .space 4
sav_s4: .space 4
sav_ret_which_way_better: .space 4
sav_s1_which_way_better: .space 4
sav_s2_which_way_better: .space 4
sav_a0_which_way_better: .space 4
sav_a1_which_way_better: .space 4

.text
main:
    #allow interrupts
    li    $t4, REQUEST_RADAR_INT_MASK        # timer interrupt enable bit
    or    $t4, $t4, REQUEST_PUZZLE_INT_MASK    # bonk interrupt bit
...
infinite:
    ...
    lw $t1, SCAN_FLAG
    beq $t1, 1, skip_req_radar #sacnning

    beq $t1, 2333, catch_star_avoid_banana  #finish_scan
    li $t0, 1
    sw $t0, SCAN_FLAG
    la $t0, star_data
    sw $t0, REQUEST_RADAR
...
skip_req_radar:
    ...
    jal go_by_jetstream
    lw $t7, STARCOIN
    bge $t7, 4, branch_puzzle
    # li $t0, 1

    lw $t0, start_solving
    bne $t0, $zero, solve_puzzle
    j    infinite
...
get_distance_from_center:
    ...
    lw $t0, jetstream_x2
    lw $t1, jetstream_y2
    lw $t2, jetstream_x2y2
    mul $v0, $a0, $t1
    mul $a1, $a1, $t0
    sub $v0, $v0, $t2
    add $v0, $a1, $v0
    jr $ra
#end get_distance_from_center
...
should_avoid:
    ...
    jal sb_arctan
    sub $s2, $s2, $v0
    bgt $s2, 270, should_avoid_yes
    blt $s2, -270, should_avoid_no
    bgt $s2, $zero, should_avoid_no
should_avoid_yes:
    lw $a0, BOT_X
    lw $a1, BOT_Y
    sub $a0, $a0, $s0
...
avoid_banana:
    ...
    move $s1, $a1
    jal should_avoid
    beq $v0, $zero, avoid_banana_loop_end
avoid_banana_loop:
    lw $t0, BOT_X
    lw $t1, BOT_Y
    sub $a0, $t0, $s0
    ...
    bge $v0, 8, avoid_banana_by_jetstream
    lw $a0, BOT_X
    lw $a1, BOT_Y
    jal get_distance_from_center
    move $s2, $v0
    move $a0, $s0
    move $a1, $s1
    jal get_distance_from_center
    sub $a0, $s0, 150
    sub $a1, $s1, 150
    bge $s2, $v0, avoid_banana_go_outer
    sub $a0, $zero, $a0
    sub $a1, $zero, $a1
avoid_banana_go_outer:
    jal sb_arctan
    li $t7, 1
    sw $v0, ANGLE
    sw $t7, ANGLE_CONTROL
...
avoid_banana_by_jetstream:
    ...
    jal sb_arctan
    sub $s2, $s2, $v0
    bgt $s2, 270, avoid_banana_skip
    blt $s2, -270, avoid_banana_angle
    bgt $s2, $zero, avoid_banana_angle
avoid_banana_angle:
    lw $t0, BOT_X
    lw $t1, BOT_Y
    sub $a0, $t0, $s0
    sub $a1, $t1, $s1
    jal euclidean_dist
    bgt $v0, 10, avoid_banana_loop_end
avoid_banana_skip:

    jal go_by_jetstream2
    j avoid_banana_loop
avoid_banana_loop_end:
...
solve_puzzle:
    ...
    sw $zero, PRINT_INT_ADDR

    sw $zero, start_solving
    sw $zero, VELOCITY
    la $s0, puzzle
...
jetstream_init:
    li $t0, 150
    # la $t3, jetstream_map
jetstream_init_loop1:
    bge $t0, 300, jetstream_init_loop1_end
    li $t1, 45000
    add $t1, $t1, $t0
    # add $t1, $t1, $t3
    lbu $t1, jetstream_map($t1)
    beq $t1, 2, jetstream_init_loop1_end
    add $t0, $t0, 2
    j jetstream_init_loop1
jetstream_init_loop1_end:
    sub $t0, $t0, 150
    sw $t0, jetstream_x
    mul $t0, $t0, $t0
    sw $t0, jetstream_x2
    ...
    li $t0, 150
jetstream_init_loop2:
    bge $t0, 300, jetstream_init_loop2_end
    mul $t1, $t0, 300
    add $t1, $t1, 150
    # add $t1, $t1, $t3
    lbu $t1, jetstream_map($t1)
    beq $t1, 2, jetstream_init_loop2_end
    add $t0, $t0, 2
    j jetstream_init_loop2
jetstream_init_loop2_end:
    sub $t0, $t0, 150
    sw $t0, jetstream_y
    mul $t0, $t0, $t0
    sw $t0, jetstream_y2
    lw $t1, jetstream_x2
    mul $t1, $t0, $t1
    sw $t1, jetstream_x2y2
    jr $ra
#end jetstream_init

catch_star_avoid_banana:
    sw $0, SCAN_FLAG
    #calc
    jal loop_find_banana_addr
    lw $t0, 0($v0)
    beq $t0, -1, no_banana
    lw $t1, 4($v0)

    srl $s1, $t0, 16 # s1 : x of banana
    and $s2, $t0, 0xffff # s2 : y of banana

    srl $s3, $t1, 16 # s3 : x of banana
    and $s4, $t1, 0xffff # s4 : y of banana

    move $a0, $s1
    move $a1, $s2
    jal should_avoid
    beq $v0, $zero, catch_star_avoid_banana_skip_first
    move $a0, $s1
    move $a1, $s2
    jal avoid_banana
    j no_banana
catch_star_avoid_banana_skip_first:
    beq $s3, 0xffff, no_banana
    move $a0, $s3
    move $a1, $s4
    jal should_avoid
    beq $v0, $zero, no_banana
    move $a0, $s3
    move $a1, $s4
    jal avoid_banana
    j no_banana
    ...
    sub $a1, $t0, $s2
    jal sb_arctan
    add $s0, $v0, 0 #s0 is now absolute angle of the banana
    slt $t0, $s0, 0
    mul $t0, $t0, 360
    add $s0, $s0, $t0
    ...
    sub $a1, $t0, $t2
    jal sb_arctan
    add $s7, $v0, 0 #s7 is now absolute angle of bot
    slt $t0, $s7, 0
    mul $t0, $t0, 360
    add $s7, $s7, $t0
    ...
    move $a0, $s1
    move $a1, $s2
    jal keep_radius_avoid

no_banana:
  # lw $t0, skip_star_flag
  # beq $t0, 1, not_worth_not_ex
   lw $a0, star_data
   bne $a0, -1, EXIST_STARCOIN
   not_worth_not_ex:
   j skip_req_radar
   EXIST_STARCOIN:
   lw $a0, star_data
   jal star_worth_collect
    beq $v0, 0, not_worth_not_ex
   worth_collect:
   lw $t1, star_data
   #get star addr
   srl $a0, $t1, 16
...
go_by_jetstream:
    ...
    mul $t2, $t1, 300
    add $t2, $t2, $t0
    lbu $t2, jetstream_map($t2)
    beq $t2, 2, go_by_jetstream_skip_if
    jal go_into_jetstream
    lw $ra, 0($sp)
    add $sp, $sp, 4
    jr $ra

go_by_jetstream_skip_if:
    sub $t0, $t0, 150
    sub $t1, $t1, 150
    lw $t2, jetstream_x
    lw $t3, jetstream_y
...
go_into_jetstream:
    ...
    lw $t2, jetstream_x2
    lw $t3, jetstream_y2
    lw $t4, jetstream_x2y2
    mul $t0, $t0, $t3
    mul $t1, $t1, $t2
    sub $t0, $t0, $t4
    add $t0, $t0, $t1
    bgt $t0, $zero, go_into_jetstream_gt0

    sub $a0, $zero, $a0
    sub $a1, $zero, $a1

go_into_jetstream_gt0:
...
move_exact_loop:
    ...
    move $a1, $s1
    jal go_toward
    bne $v0, $zero, move_exact_loop_end
...
keep_radius_avoid:
    ...
    move $s5, $a0
    move $s6, $a1
    jal calc_absoulte_deg
    move $s3, $v0 # s3 is the degree of banana

    move $a0, $s5
    move $a1, $s6

    jal calc_radius_of_center
    move $s7, $v0 #s7 is radius of banana

    lw $a0, BOT_X
    lw $a1, BOT_Y
    jal calc_radius_of_center
    move $s6, $v0    # s6 is radius of bot

    add $s1, $s6, 8 # if Rbot + 8 > R_banana then decrease radius
    blt $s1, $s7, loop_radius_bef
    dec_radius_to_center:
        li $a0, 150
        li $a1, 150
        li $t0, 10
        sw $t0, VELOCITY # v = 10
        ...
        jal calc_radius_of_center
        add $t0, $v0, 10
        bgt $t0, $s7, dec_radius_to_center
loop_radius_bef:
        li $t0, 10
        sw $t0, VELOCITY # v = 10
        ...
        lw $a1, BOT_Y
        jal calc_radius_of_center
        move $s5, $v0 #s5 is the radius set
loop_radius:
        lw $a0, BOT_X
        lw $a1, BOT_Y
        li $t0, 150
        ...
        sub $a1, $t0, $a1
        jal sb_arctan
        sub $s7, $v0, 90 #s7 is the deg used
        lw $a0, BOT_X
        lw $a1, BOT_Y
        jal calc_radius_of_center
        move $t4, $v0 # t4 is the radius now
        #if radius now > Rset
        sub $a0, $t4, $s5
        bgt $a0, 2, add_deg
        bgt $a0, -2, end_if_deg
...
calc_absoulte_deg:
    ...
    slt $t0, $v0, 0
    mul $t0, $t0, 360
    add $v0, $v0, $t0 # v0 is the absolute deg of banana
...
loop_find_banana_addr:
    la  $v0, star_data
try_find:
    lw $t0, 0($v0)
    beq $t0, -1, found_banana
        add $v0, $v0, 4
        j try_find
found_banana:
    add $v0, $v0, 4
...
which_way_better:
    sw $ra, sav_ret_which_way_better
    sw $s1, sav_s1_which_way_better
    sw $s2, sav_s2_which_way_better
    sw $a0, sav_a0_which_way_better
    sw $a1, sav_a1_which_way_better
    ...
    sub $a1, $t0, $t2
    jal euclidean_dist
    move $s2, $v0 # s2 is R(BOT)

    bgt $s2, $s1, RBOT_RBANAN
        li $v0, 1
        j END_WHICH_BETTER_IF
RBOT_RBANAN:
        li $v0, 0
END_WHICH_BETTER_IF:

    lw $ra, sav_ret_which_way_better
    lw $s1, sav_s1_which_way_better
    lw $s2, sav_s2_which_way_better
    lw $a0, sav_a0_which_way_better
    lw $a1, sav_a1_which_way_better
    jr $ra
...
interrupt_dispatch:            # Interrupt:
    ...
    and    $a0, $k0, REQUEST_RADAR_INT_MASK    # is there a radar interrupt?
